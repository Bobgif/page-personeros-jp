...
const RegistroForm = () => {
  const [datos, setDatos] = useState<DatosPersonero>({
    dni: "", nombres: "", apellidos: "", provincia: "", distrito: "", 
    centro: "", telefono: "", correo: "", invitado: ""
  });
  ...
  const verificarDni = async () => {
    ...
    if (res.success && res.nombres) {
      ...
      switch(res.error) {
        case "LÍMITE_DIARIO":
          setMensaje("🚫 Solo puedes registrar a una persona por día. ¡Vuelve mañana!");
          break;
        case "YA_REGISTRADO":
          setMensaje("🚫 Ya estás registrado como personero. ¡Gracias por tu apoyo!");
          break;
        case "FUJIBOT":
          setMensaje("🚫 Fujibot detectado. DNI no existe en RENIEC.");
          break;
        default:
          setMensaje(res.error || "🚫 Error inesperado en el búnker.");
      }
    }
  };
    ...
    <form onSubmit={enviarRegistro} className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border border-slate-100">
      ...
      <div className="flex flex-row items-center gap-2 mb-4 w-full">
        ...
        <button 
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-xl font-bold transition-transform active:scale-95 shadow-lg shadow-blue-200 whitespace-nowrap text-sm"
        >
          Validar
        </button>
      </div>
      ...
      <div className={`space-y-4 transition-all duration-500 ${!verificado ? "opacity-30 grayscale pointer-events-none" : "opacity-100"}`}>
        ...
        <UbigeoSelects verificado={verificado} datos={datos} setDatos={setDatos} />

        <input 
          type="text" required inputMode="numeric" placeholder="Celular" maxLength={9} 
        ...
        <input 
          type="text" placeholder="¿Alguien te invitó?" 
        ...
        <button 
          type="submit" disabled={!verificado} 
